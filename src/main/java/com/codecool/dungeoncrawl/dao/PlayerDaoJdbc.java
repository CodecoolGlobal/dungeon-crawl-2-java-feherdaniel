...
public class PlayerDaoJdbc implements PlayerDao {
    ...
    public PlayerModel get(int id) {
        try (Connection conn = dataSource.getConnection()) {
            String sql = "SELECT player_name, hp, x, y, inventory FROM player WHERE id = ?";
            ...
            PlayerModel playerModel = new PlayerModel(rs.getString(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getString(5));
    ...
    public List<PlayerModel> getAll() {
        try (Connection conn = dataSource.getConnection()) {
            ...
            while (rs.next()) { // while result set pointer is positioned before or on last row read authors
                PlayerModel playerModel = new PlayerModel(rs.getString(2), rs.getInt(3), rs.getInt(4), rs.getInt(5), rs.getString(6));
